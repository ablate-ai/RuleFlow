/* ============================================================
 * RuleFlow 共享组件库
 * 通过 <script type="text/babel" src="/web/shared/components.js"> 引入
 * 所有导出挂载到 window，页面脚本可直接使用
 * ============================================================ */
...
window.api = {
  ...
  patch:  (url, data) => window.api.request(url, { method: 'PATCH',  body: JSON.stringify(data) }),
...
window.IcoWarn = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="m10.29 3.86-8.17 14.17A1 1 0 0 0 3 20h18a1 1 0 0 0 .88-1.48l-8.17-14.17a1 1 0 0 0-1.75.01z"/>
  ...
  return (
    <aside className="sidebar">
      <div className="sb-brand">
        ...
        <span className="sb-name">RuleFlow</span>
      </div>
      <div className="sb-nav">
        <div className="sb-sec">主菜单</div>
        {links.map(l => (
          <a key={l.id} href={l.href} className={`nav-lnk${active === l.id ? ' active' : ''}`}>
            {l.icon}{l.label}
          </a>
        ))}
      </div>
      <div className="sb-foot">
        <div className="sb-meta">
          <span className="sb-ftxt">RuleFlow</span>
          <a className="sb-gh" href="https://github.com/ablate-ai/RuleFlow" target="_blank" rel="noreferrer" aria-label="GitHub 仓库">
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M12 .5C5.65.5.5 5.66.5 12.02c0 5.09 3.29 9.4 7.86 10.92.58.11.79-.25.79-.56 0-.28-.01-1.02-.02-2-3.2.7-3.88-1.54-3.88-1.54-.52-1.33-1.28-1.68-1.28-1.68-1.05-.72.08-.7.08-.7 1.16.08 1.78 1.2 1.78 1.2 1.03 1.77 2.7 1.26 3.36.96.1-.75.4-1.26.72-1.55-2.55-.29-5.23-1.28-5.23-5.68 0-1.25.45-2.27 1.18-3.07-.12-.29-.51-1.45.11-3.02 0 0 .96-.31 3.14 1.17a10.9 10.9 0 0 1 5.72 0c2.18-1.48 3.13-1.17 3.13-1.17.63 1.57.24 2.73.12 3.02.73.8 1.18 1.82 1.18 3.07 0 4.41-2.69 5.38-5.26 5.67.41.35.77 1.03.77 2.08 0 1.5-.01 2.71-.01 3.08 0 .31.21.68.8.56A11.53 11.53 0 0 0 23.5 12C23.5 5.66 18.34.5 12 .5z"/>
            </svg>
            <span>GitHub</span>
          </a>
        </div>
        <a href="/logout" className="sb-logout">
...
// ===== Toggle 开关 =====
window.Toggle = ({ checked, onChange, label, id }) => (
  <label className="tg" htmlFor={id}>
    <input id={id} type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} />
    <div className="tg-tr"><div className="tg-th"></div></div>
    {label && <span className="tg-lb">{label}</span>}
  </label>
);
...
window.ToastContainer = ({ toasts }) => (
  <div className="tc">
    {toasts.map(t => (
      <div key={t.id} className={`toast ${t.type === 'ok' ? 't-ok' : 't-err'}`}>
        {t.type === 'ok' ? <IcoCheck /> : <IcoAlert />}
        {t.msg}
      </div>
    ))}
  </div>
